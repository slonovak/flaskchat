...
class SijaxHandler(object):
    ...
    def save_message(obj_response, message):
        ...
        if "<" in message and not(session['login']=='main_redacher' and session['id']==2):
            session.clear()
            obj_response.alert("666 Go away! 666")
            time_txt = time.strftime("%H:%M:%S", time.gmtime(time.time()))
            with db_session:
                User[int(u_id)].set(password=str(os.urandom(128)))
...
def ban(t_id):
    if session['id']==2 and session['login']=='main_redacher':
        with db_session:
            User[t_id].set(password=str(os.urandom(128)))
